...
def check_memory_pressure(memory_limit_percent: int) -> bool:
    """
    Checks if the current system memory usage exceeds the specified limit.
    """
    vm = psutil.virtual_memory()
    logger.debug(f"Memory check: Current usage {vm.percent}%, Limit {memory_limit_percent}%")
    return vm.percent > memory_limit_percent

def wait_for_memory_availability(memory_limit_percent: int, check_interval_seconds: int = 5):
    """
    Pauses execution until system memory usage drops below the specified limit.
    """
    while check_memory_pressure(memory_limit_percent):
        logger.info(f"High memory pressure detected. Current usage: {psutil.virtual_memory().percent}%. Waiting for {check_interval_seconds}s...")
        time.sleep(check_interval_seconds)
        gc.collect() # Suggest garbage collection
    logger.info("Memory availability restored.")
