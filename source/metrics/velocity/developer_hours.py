...
class DeveloperHoursMetric(BaseMetric):
    ...
    def get_metrics(self):
        """Get developer hours metrics."""
        # First calculate all sessions
        ...
        for developer, stats in self.developer_stats.items():
            metrics[developer] = {
                'total_estimated_hours': round(stats['total_hours'], 2),
            ...
            for week, week_stats in stats['weekly_hours'].items():
                ...
                if sessions > 0:
                    avg_session = week_stats['estimated_hours'] / sessions
                else:
                    avg_session = 0
                
                metrics[developer]['weekly_hours'][week] = {
                    'estimated_hours': round(week_stats['estimated_hours'], 2),
                    'sessions': sessions,
                    'commits': week_stats['commits'],
                    'avg_session_hours': round(avg_session, 2),
                    'productive_days': len(week_stats['productive_days']),
                    'hours_per_day': round(
                        week_stats['estimated_hours'] / max(1, len(week_stats['productive_days'])), 
                        2
                    )
                }
        ...
        for metrics in metrics_list:
            for developer, dev_stats in metrics.items():
                ...
                for week, week_stats in dev_stats.get('weekly_hours', {}).items():
                    for key in ['estimated_hours', 'sessions', 'commits', 'productive_days']:
                        merged[developer]['weekly_hours'][week][key] += week_stats.get(key, 0)
        
        # Recalculate averages after merging
        result = {}
        for developer, dev_stats in merged.items():
            result[developer] = {
                'total_estimated_hours': round(dev_stats['total_estimated_hours'], 2),
                'total_sessions': dev_stats['total_sessions'],
                'weekly_hours': {}
            }
            
            for week, week_stats in dev_stats['weekly_hours'].items():
                sessions = week_stats['sessions']
                days = week_stats['productive_days']
                
                result[developer]['weekly_hours'][week] = {
                    'estimated_hours': round(week_stats['estimated_hours'], 2),
                    'sessions': sessions,
                    'commits': week_stats['commits'],
                    'avg_session_hours': round(week_stats['estimated_hours'] / max(1, sessions), 2),
                    'productive_days': days,
                    'hours_per_day': round(week_stats['estimated_hours'] / max(1, days), 2)
                }
